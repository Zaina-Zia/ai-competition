 ...
 *
 * **Crucially, operations like `listAll` (used in `getAllStoredArticles`) trigger a preflight `OPTIONS` request
 * before the actual `GET` request.** Your bucket MUST be configured to allow BOTH `GET` and `OPTIONS` methods
 * from your web app's origin(s). The error "Response to preflight request doesn't pass access control check:
 * It does not have HTTP ok status" specifically points to this preflight failure.
 *
 ...
 *           "https://9000-idx-studio-1746337412493.cluster-w5vd22whf5gmav2vgkomwtc4go.cloudworkstations.dev", // Specific dev URL (Port 9000)
 ...
 *    c. Go to the "Permissions" tab, then "Edit access" or find the CORS section.
 *    d. Add your origin(s) (e.g., `https://6000-idx-studio-...`).
 *    e. Select BOTH `GET` and `OPTIONS` methods.
 *    f. Add `Content-Type` and `Access-Control-Allow-Origin` to `responseHeader`.
 *    g. Set `maxAgeSeconds` (e.g., 3600). Save.
 *
 * IMPORTANT NOTES:
 *  - It might take a few minutes for CORS changes to propagate. Clear browser cache/hard reload.
 *  - Ensure the `origin` EXACTLY matches the origin in the browser's address bar.
 *
...
const FETCH_TIMEOUT_MS = 15000; // Increased timeout for individual fetches (15 seconds)
const LIST_TIMEOUT_MS = 20000; // Increased timeout for listing files (20 seconds)
    ...
    try {
        ...
        let errorMessage = `Failed to fetch article data for ${articleId}. `;
         if (error.name === 'AbortError' || (error instanceof Error && error.message.includes('timed out'))) {
            ...
            errorMessage += 'This might be a network issue or a CORS configuration problem on the storage bucket. Verify that the origin of your application is in the allowed origins in the Firebase Storage CORS configuration and that the GET method is allowed.';
...
export async function getAllStoredArticles(): Promise<StoredArticleData[]> {
    ...
    try {
        // *** CORS ERROR LIKELY HAPPENS HERE if OPTIONS method is not allowed for the origin ***
        // listAll triggers a preflight OPTIONS request. It MUST succeed.
        ...
        if (jsonFiles.length === 0) {
             console.warn('No article JSON files found in storage. Ensure the "articles" folder exists and contains .json files, or check Storage Rules.');
        ...
        const fetchPromises = jsonFiles.map(async (itemRef: StorageReference) => {
            ...
            try {
                ...
                const result = await Promise.race([
                 ...
                 if (result === null) {
                    // This case should ideally not happen if timeout throws error, but added for safety
                    console.warn(`getArticle for ${articleId} resulted in null, potentially due to timeout.`);
                    return null;
                 }
                 return result;
            } catch (error: any) {
                // Log specific errors for each article that fails (including timeouts)
                console.error(`Failed to get/process article with ID ${articleId} (${itemRef.name}):`, error?.message || error);
        ...
        // Increase overall timeout for Promise.all to allow for concurrent fetches
        const allResults = await Promise.race([
           Promise.all(fetchPromises),
           new Promise<null[]>((_, reject) =>
             setTimeout(() => reject(new Error(`getAllStoredArticles timed out after ${LIST_TIMEOUT_MS / 1000} seconds waiting for all articles.`)), LIST_TIMEOUT_MS)
           )
         ]);

         if (!allResults) {
             // This means the overall LIST_TIMEOUT_MS was hit.
             // This is unlikely if individual fetches have shorter timeouts, but possible.
             console.error(`getAllStoredArticles timed out globally after ${LIST_TIMEOUT_MS / 1000} seconds.`);
             throw new Error(`Failed to fetch all articles within the time limit.`);
         }

        // Filter out any null results from failed fetches and log counts
        const successfulArticles = allResults.filter((article): article is StoredArticleData => article !== null);
        const failedCount = allResults.length - successfulArticles.length;
        if (failedCount > 0) {
             console.warn(`Failed to fetch content for ${failedCount} out of ${allResults.length} articles due to errors or timeouts. Check previous logs and CORS configuration.`);
        }
        ...
        if (error.code === 'storage/retry-limit-exceeded') {
            errorMessage += `Max retry time exceeded. This might indicate network issues or problems reaching the storage service.`;
             ...
             (error.code === 'storage/unknown' && error.message.includes('NetworkError')) ||
             (error instanceof TypeError && error.message.includes('Failed to fetch')) || // Catching fetch error from listAll itself
             (error instanceof Error && error.message.includes("preflight request doesn't pass access control check")) // Explicit check for the CORS preflight error
         ) {
             // This specific error pattern often indicates a CORS failure on the OPTIONS preflight request for listAll.
             errorMessage += `A network error occurred, VERY LIKELY DUE TO CORS. The 'listAll' operation requires the 'OPTIONS' method to be allowed in your Firebase Storage CORS configuration for the origin '${window.location.origin}'. Please verify your CORS settings include 'OPTIONS'.`;
            ...
            // This shouldn't happen with listAll on a folder, but good to handle.
             ...
             errorMessage += `The operation timed out after ${LIST_TIMEOUT_MS / 1000} seconds. Check network or increase LIST_TIMEOUT_MS.`;
         }
        else {
